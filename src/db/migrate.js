...
async function migrate() {
  ...
  const existing = await query('SELECT COUNT(*) AS count FROM roles');
  if (existing[0].count === 0) {
    await query('INSERT INTO roles (name, description) VALUES ?', [seedRoles]);
  }
  ...
  if (usersCount[0].count === 0) {
    ...
    await query(
      ...
      [[
        ['Admin User', 'admin@pse.com', hashed, 'General Admin', 'Executive', 'Active'],
        ['HR Lead', 'hr@pse.com', hashed, 'HR', 'Human Resources', 'Active'],
        ['COO Leader', 'coo@pse.com', hashed, 'COO', 'Executive', 'Active'],
        ['Sales Head', 'sales@pse.com', hashed, 'Sales', 'Sales', 'Active'],
        ['Delivery Lead', 'leadpm@pse.com', hashed, 'Lead Project Manager', 'Operations', 'Active'],
        ['Project Manager', 'pm@pse.com', hashed, 'Project Manager', 'Operations', 'Active'],
        ['Consultant User', 'consultant@pse.com', hashed, 'Consultant', 'Engineering', 'Active']
      ]]
    );
  }
